...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (supplierToAdd) {
            ...
            // We ensure it exists but do NOT add it to the product's main categories
             console.log(`[API CREATE][AUDIT] Ensured supplier category exists with ID: ${supplierCatId}`);
        }
        ...
        if (tagNames.length > 0) {
            ...
            finalProductData.tags = tagIds.map(tagId => ({ id: tagId })) as any;
        } else {
            finalProductData.tags = [];
        }
