...
function autopress_ai_register_rest_endpoints() {
    ...
    function custom_api_get_content_list($request) {
        ...
        if ($post_query->have_posts()) {
            foreach ($post_query->posts as $post_obj) {
                $type_slug = get_post_type($post_obj->ID);
                $type_label = $type_slug === 'page' ? 'Page' : 'Post';
                $is_front = in_array($post_obj->ID, $all_front_page_ids);

                $content_list[] = [
                    'id' => $post_obj->ID, 'title' => $post_obj->post_title, 'slug' => $post_obj->post_name,
                    'type' => $type_label, 'link' => get_permalink($post_obj->ID), 'status' => $post_obj->post_status,
                    ...
                    'modified' => $post_obj->post_modified, 'is_front_page' => $is_front,
                ];
            }
        }
        ...
        foreach ($all_terms as $term) {
             $content_list[] = [
                'id' => $term->term_id, 'title' => $term->name, 'slug' => $term->slug,
                'type' => 'Categoría de Entradas', 'link' => get_term_link($term), 'status' => 'publish',
                'parent' => $term->parent,
                ...
                'modified' => current_time('mysql'), 'is_front_page' => false,
            ];
        }
    ...
    function custom_api_get_or_create_category(WP_REST_Request $request) {
        ...
        foreach ($path_parts as $part) {
            ...
            // Only add lang if polylang is active and a lang is passed
            if (function_exists('pll_get_language') && $lang_slug) {
                $args['lang'] = $lang_slug;
            }
