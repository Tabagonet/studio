...
// === REST API Endpoints ===
// Register endpoints regardless of the key verification status.
// The permission_callback on each endpoint will handle security.
...
function autopress_ai_register_rest_endpoints() {
    ...
    function custom_api_status_check() { return new WP_REST_Response( ['status' => 'ok', 'plugin_version' => '1.22'], 200 ); }
    ...
    function custom_api_get_content_list($request) {
        // Whitelist approach: only query for the specific post types we want.
