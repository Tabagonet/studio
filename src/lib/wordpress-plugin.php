...
function custom_api_get_polylang_languages() {
    ...
    // Use pll_languages_list to get all defined languages, not just those with content.
    ...
    foreach ($language_slugs as $slug) {
        $lang_details = pll_get_language($slug, 'name'); // Fetch full name
        $formatted_languages[] = [
            'code' => $slug,
            'name' => $lang_details ? $lang_details : $slug,
        ];
    }
