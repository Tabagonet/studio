 ...
 const handlePhotosWithAutoName = useCallback((newPhotos: ProductPhoto[]) => {
  ...
  const handleSourceLanguageChange = (newSourceLangCode: string) => {
      ...
      if(selectedLang) {
         updateProductData({
            ...
            targetLanguages: productData.targetLanguages?.filter(l => l !== selectedLang.code)
         });
      }
  };
  ...
  const handleGenerateImageMetadata = async () => {
    ...
    try {
        ...
        if (!response.ok) {
            let errorToastDescription = `El servidor respondió con un error ${response.status}.`;
            try { const errorData = await response.json(); errorToastDescription = errorData.error || errorData.message || errorToastDescription; } catch (e) {}
            throw new Error(errorToastDescription);
        }
        const aiContent = await response.json();
        updateProductData({
                ...
                <CardContent className="space-y-6">
                  ...
                  {nameStatus.status === 'exists' && (
                    isDuplicateButResolved ? (
                       <Alert variant="success">
                        ...
                        <AlertDescription>
                          Has proporcionado un SKU único y un proveedor, ¡perfecto! Se generará un slug único para diferenciarlo.
                        </AlertDescription>
                      </Alert>
                    ) : (
                      <Alert variant="destructive">
                        ...
                        <AlertDescription>
                          Para crear el producto, ahora será **obligatorio** que rellenes los campos **Proveedor** y **SKU** con valores únicos para diferenciarlo y evitar problemas de SEO.
                        </AlertDescription>
                      </Alert>
                    )
                  )}
                  ...
                  <div>
                    ...
                    <Select name="productType" value={productData.productType} onValueChange={(value) => handleSelectChange('productType', value)} disabled={isProcessing}>
                      <SelectTrigger id="productType"><SelectValue placeholder="Selecciona un tipo de producto" /></SelectTrigger>
                      <SelectContent>{PRODUCT_TYPES.map(type => (<SelectItem key={type.value} value={type.value}>{type.label}</SelectItem>))}</SelectContent>
                    </Select>
                  </div>
                  ...
                  {productData.productType !== 'grouped' && (
                      <div className="border-t pt-6 mt-6">
                          ...
                          <Button type="button" variant="outline" onClick={addAttribute} className="mt-2" disabled={isProcessing || isGenerating}><PlusCircle className="mr-2 h-4 w-4" /> Añadir Atributo</Button>
                      </div>
                  )}
              ...
              <Card><CardHeader><CardTitle>Descripciones y Etiquetas</CardTitle><CardDescription>Esta información es clave para el SEO y para informar a tus clientes.</CardDescription></CardHeader>
                <CardContent className="space-y-6">
                  ...
                  <div className="border-t pt-6 space-y-6">
                    ...
                    <div><Label htmlFor="longDescription">Descripción Larga</Label><RichTextEditor content={productData.longDescription} onChange={handleLongDescriptionChange} onInsertImage={() => setIsImageDialogOpen(true)} onSuggestLinks={handleSuggestLinks} placeholder="Describe tu producto en detalle: características, materiales, usos, etc."/></div>
                  </div>
                </CardContent>
              </Card>
          </div>
          <div className="lg:col-span-1 space-y-8">
              <Card>
                ...
                <CardContent className="space-y-4">
                  <ImageUploader photos={productData.photos} onPhotosChange={handlePhotosWithAutoName} isProcessing={isProcessing || isGenerating} maxPhotos={15} onCropImage={onCropImage} />
