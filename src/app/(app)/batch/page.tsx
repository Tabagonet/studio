...
export default function BatchProcessingPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe: Unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      if (allTerminated) {
        ...
        // No statuses yet for this batchId, but currentBatchId is set, so assume backend will start.
        setIsBackendProcessing(true); 
      }
      ...
      toast({ title: "Error de Sincronización", variant: "destructive", description: "No se pudieron obtener los estados del lote." });
      setIsBackendProcessing(false);
      setInitialLoadComplete(true);
    });
  ...
  }, [currentBatchId, toast]); // batchPhotosStatus removed from deps to avoid potential loops if not careful with state updates
  ...
  const handleStartUploads = async () => {
    ...
    if (productsToProcess.length === 0) {
      toast({ title: "No hay imágenes o productos detectados", variant: "destructive", description: "Por favor, sube imágenes o asegúrate de que los nombres sigan un patrón." });
      return;
    }
    ...
    for (const productGroup of productsToProcess) {
      ...
      const baseProductContext: Omit<WizardProductContext, 'isPrimary' | 'sku' | 'regularPrice' | 'salePrice' | 'category' | 'keywords' | 'shortDescription' | 'longDescription' | 'attributes' > & Partial<WizardProductContext> = {
        ...
        sku: cleanTextForSku(productName), 
      ...
      for (let i = 0; i < productGroup.photoObjects.length; i++) {
        ...
        if (uploadResult && uploadResult.success && uploadResult.relativePath) {
          ...
          firestoreBatch.set(photoDocRef, {
            ...
            originalStoragePath: uploadResult.relativePath, 
            originalDownloadUrl: uploadResult.relativePath, 
            status: "uploaded",
    ...
    if (firestoreWriteCount > 0) {
      try {
        ...
        toast({ title: "Error de Base de Datos", description: "No se pudieron guardar los detalles de las imágenes.", variant: "destructive", duration: 10000 });
        setIsUploading(false);
        return; 
      }
    }
  ...
  const getStatusText = (status: ProcessingStatusEntry['status']): string => {
    // This map directly reflects the statuses set by the backend in /api/process-photos
    const map: Partial<Record<ProcessingStatusEntry['status'], string>> = {
        uploaded: "Subido, Pend. Proceso",
        processing_image_started: "Iniciando Proc. Imagen",
        processing_image_downloaded: "Cargando Imagen Local (Backend)",
        ...
        processing_image_optimized: "Optimizando Imagen (Sharp)",
        processing_image_rules_applied: "Aplicando Reglas de Aut.",
        completed_image_pending_woocommerce: "Imagen Lista, Pend. Producto Woo",
        error_processing_image: "Error Procesando Imagen",
        completed_woocommerce_integration: "Producto Creado en WooCommerce",
        error_woocommerce_integration: "Error Creando Producto Woo"
    };
