...
import { KeyRound, Save, Loader2, Trash2, PlusCircle, Users, Building, User, Globe, Store, PlugZap, AlertCircle, CheckCircle, Shield } from "lucide-react";
...
export default function ConnectionsPage() {
    ...
    const [partnerStatus, setPartnerStatus] = useState<'idle' | 'verifying' | 'verified' | 'error'>('idle');
    ...
    const handleVerifyPartnerConnection = async () => {
        setPartnerStatus('verifying');
        ...
        try {
            const token = await user.getIdToken();
            // This API call implicitly uses the saved credentials for the entity.
            ...
            if (!response.ok) {
                throw new Error(result.error || "Fallo la verificación");
            }

            setPartnerStatus('verified');
            toast({ title: "¡Conexión Verificada!", description: "Las credenciales de Shopify Partner son correctas." });
    ...
    const handlePartnerInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const { name, value } = e.target;
        setPartnerStatus('idle'); // Reset status on change
    ...
    const handleSave = async (isPartnerCreds: boolean = false) => {
        ...
        try {
            ...
            if (isPartnerCreds) {
                setPartnerStatus('idle'); // Reset verification status after saving
            }
            setRefreshKey(k => k + 1);
            if (!isPartnerCreds) {
    ...
    const handleDelete = async (keyToDelete: string) => {
        ...
        try {
            ...
            if (keyToDelete === 'shopify_partner') {
                setPartnerStatus('idle');
            }
            window.dispatchEvent(new Event('connections-updated'));
            await fetchConnections(user, editingTarget.type, editingTarget.id);
        ...
        <div className="container mx-auto py-8 space-y-8">
            ...
            <Card>
                ...
                <CardContent className="space-y-4">
                     <ConnectionStatusIndicator status={selectedEntityStatus} isLoading={isCheckingStatus} onRefresh={() => setRefreshKey(prev => prev + 1)} />
                        ...
                        <Card className="mt-8 border-primary/50">
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div className="flex items-center gap-2 flex-wrap">
                                    <Button onClick={() => handleSave(true)} disabled={isSavingPartner || partnerStatus === 'verifying'}>
                                        {(isSavingPartner || partnerStatus === 'verifying') && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                                        Guardar Credenciales
                                    </Button>
                                    <Button variant="outline" onClick={handleVerifyPartnerConnection} disabled={isSavingPartner || !partnerFormData.partnerClientId || partnerStatus === 'verifying'}>
                                        {partnerStatus === 'verifying' ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Shield className="mr-2 h-4 w-4" />}
                                        Verificar Conexión
                                    </Button>

                                    {partnerStatus === 'verified' && <Badge variant="default" className="bg-green-600 hover:bg-green-700"><CheckCircle className="mr-1 h-3 w-3" />Verificado</Badge>}
                                    {partnerStatus === 'error' && <Badge variant="destructive"><AlertCircle className="mr-1 h-3 w-3" />Error de Verificación</Badge>}
