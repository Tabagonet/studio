...
const ConnectionStatusIndicator = ({ status, isLoading }: { status: SelectedEntityStatus | null, isLoading: boolean }) => {
  ...
  const isWooCommerceActive = status.activePlatform === 'woocommerce';
  const isPluginVerified = isWooCommerceActive && status.pluginActive;

  if (isWooCommerceActive && !isPluginVerified) {
     return (
        <div className="flex items-center gap-2 text-sm text-destructive border border-destructive/20 p-3 rounded-md bg-destructive/10">
            <AlertCircle className="h-4 w-4" />
            <span>Conexión a <strong>{hostname}</strong> no verificada. Revisa la API Key en el plugin de WordPress.</span>
        </div>
      );
  }
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
                {isWooCommerceActive && (
                    <div className="flex items-center gap-2">
                        <Tooltip>
                            <TooltipTrigger>
                            <Store className={cn("h-4 w-4", status.wooCommerceConfigured ? "text-green-500" : "text-muted-foreground")} />
                            </TooltipTrigger>
                            <TooltipContent>
                        ...
                        <Tooltip>
                            ...
                            <TooltipContent>
                                <p>Plugin AutoPress AI: {status.pluginActive ? "Activo y Verificado" : "No Detectado o No Verificado"}</p>
                            </TooltipContent>
                        </Tooltip>
                    </div>
                )}
