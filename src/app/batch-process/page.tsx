...
import { UploadCloud, FileText, Info, FileSpreadsheet, Image as ImageIcon, CheckCircle, Download, Loader2, FileWarning, PackageCheck, PackageX, PackageSearch, Sparkles, ShieldAlert, ExternalLink } from "lucide-react";
...
export default function BatchProcessPage() {
  ...
  const updateProductProcessingStatus = (
    ...
    setStagedProducts(prev => 
      prev.map(p => 
        p.id === sku 
          ? { ...p, processingStatus, processingMessage: processingMessage || p.processingMessage, progress: progress !== undefined ? progress : p.progress, adminUrl: adminUrl || p.adminUrl } 
          : p
      )
    );
  };
    ...
    for (const product of productsToProcess) {
        ...
        try {
            ...
            if (!createResponse.ok) {
                throw new Error(`Error al crear en Woo: ${responseData.error || 'Error desconocido'}`);
            }

            updateProductProcessingStatus(product.id, 'completed', '¡Producto creado con éxito!', 100, responseData.admin_url);
            successes++;
                                ...
                                <div className="flex-1 max-w-xs space-y-2">
                                    <div className="flex items-center justify-between gap-2">
                                        <div className="flex items-center gap-2">
                                         ...
                                         {product.processingStatus === 'completed' && product.adminUrl && (
                                            <Button asChild variant="outline" size="icon-sm" className="flex-shrink-0" title="Ver en WooCommerce">
                                                <Link href={product.adminUrl} target="_blank" rel="noopener noreferrer">
