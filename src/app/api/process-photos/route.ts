...
async function generateProductDescription(
  ...
  console.warn("[API /process-photos] TEMPORARILY DISABLED: generateProductDescription (Genkit) is returning placeholders.");
  const productName = input.productName || "Producto";
  return {
    shortDescription: `Placeholder (Genkit Deshabilitado) descripción corta para ${productName}.`,
    longDescription: `Placeholder (Genkit Deshabilitado) descripción larga para ${productName}. Categoría: ${input.categoryName}, Palabras Clave: ${input.keywords}, Atributos: ${input.attributesSummary}.`,
  };
}
...
async function generateContentWithMiniLM(
  ...
  console.warn(`[API /process-photos] TEMPORARILY DISABLED: generateContentWithMiniLM (MiniLM) is returning placeholders for ${input.productName}.`);
  const baseName = cleanTextForFilename(input.productName || 'placeholder-product');
  return {
    seoFilenameBase: baseName,
    shortDescription: `Placeholder (MiniLM Deshabilitado) descripción corta para ${input.productName}.`,
    longDescription: `Placeholder (MiniLM Deshabilitado) descripción larga para ${input.productName}.`,
    seoMetadata: {
      alt: `Placeholder (MiniLM Deshabilitado) alt text para ${input.productName}`,
      title: `Placeholder (MiniLM Deshabilitado) Título: ${input.productName}`,
      description: `Placeholder (MiniLM Deshabilitado) meta descripción para ${input.productName}.`,
    },
    attributes: input.existingAttributes || [{ name: 'Atributo Placeholder', value: 'Valor Placeholder' }],
    tags: input.existingKeywords?.split(',').map(k => k.trim()).filter(k => k) || ['placeholder-tag'],
  };
}
// --- END TEMPORARY PLACEHOLDER FUNCTIONS ---
...
async function uploadImageToWooCommerceMedia(
  ...
  try {
    ...
    const fileTypeResult = await fileTypeFromBuffer(fileBuffer); // Correct usage
    ...
    const targetUrl = `${wooCommerceStoreUrl.replace(/\/$/, '')}/wp-json/wp/v2/media`;
    ...
    console.log(`[WC Media Upload - ${originalImageName}] Axios POST finished. Time: ${uploadEndTime - Date.now()}ms. Status: ${response.status}`);
    ...
    console.error(`[WC Media Upload - ${originalImageName}] Full exception during upload with Axios:`, error);
    const errorMessage = error.response?.data?.message || error.message || 'Unknown error uploading to WooCommerce.';
...
async function fetchWooCommerceCategories(forceRefresh: boolean = false): Promise<WooCommerceCategory[]> {
    ...
    try {
        ...
        if (error.response?.data) console.error("[WooCommerce Categories] Error response data:", JSON.stringify(error.response.data, null, 2));
        return allWooCategoriesCache; 
    }
}
...
async function updateSpecificFirestoreEntries(
  ...
  try {
    ...
    console.log(`[Firestore Update] Updated status for ${entryIds.length} entries to ${status}. First ID: ${entryIds[0]}, Keys updated: ${Object.keys(updateData).length > 0 ? (Object.keys(updateData) as Array<keyof ProcessingStatusEntry>).filter(k => updateData[k] !== undefined).join(', ') : 'none'}`);
  } catch (error){
...
async function createWooCommerceProductForGroup(
    ...
    if (!finalShortDescription || finalShortDescription.trim() === "" || finalShortDescription.includes("Placeholder MiniLM") || finalShortDescription.includes("Placeholder Genkit")) {
        console.log(`[WooCommerce CreateProduct - ${productNameFromContext}] Short description missing or placeholder, attempting Genkit/Template fallback.`);
        try {
            console.log(`[WooCommerce CreateProduct - ${productNameFromContext}] Invoking generateProductDescription...`);
    ...
    if (!finalLongDescription || finalLongDescription.trim() === "" || finalLongDescription.includes("Placeholder MiniLM") || finalLongDescription.includes("Placeholder Genkit")) {
        console.log(`[WooCommerce CreateProduct - ${productNameFromContext}] Long description missing or placeholder, attempting Genkit/Template fallback.`);
         try {
            console.log(`[WooCommerce CreateProduct - ${productNameFromContext}] Invoking generateProductDescription for long desc...`);
            const aiDescInput = { productName: currentProductContext.name, categoryName: templateDataForDesc.categoria, keywords: templateDataForDesc.palabras_clave, attributesSummary: templateDataForDesc.atributos };
            const aiOutput = await generateProductDescription(aiDescInput);
    ...
    console.log(`[WooCommerce CreateProduct - ${productNameFromContext}] Long Description (first 50): ${(wooProductData.description || "").substring(0,50)}...`);
    ...
    const attemptProductCreation = async (productPayload: any, isRetry: boolean): Promise<{id: number | string | null, skuUsed: string | null, errorMsg?: string, errorDetails?: any}> => {
        ...
        try {
            ...
            const response = await wooApi.post("products", productPayload, { timeout: 120000 });
...
export async function POST(request: NextRequest) {
  ...
  let originalRequestUrlString: string = "unknown_url_from_request_object";
  ...
  try {
    ...
    try {
        ...
        const rawBodyText = await request.text().catch(() => "Could not read raw body.");
        console.error("[API /process-photos] Raw request body (first 200 chars):", rawBodyText.substring(0,200));
        console.error("[API /process-photos] Request URL that might be problematic:", request.url);
    ...
    if (!adminDb || !adminAuth) {
     ...
     if (!wooApi && process.env.WOOCOMMERCE_STORE_URL && process.env.WOOCOMMERCE_API_KEY && process.env.WOOCOMMERCE_API_SECRET) {
      // Attempt to initialize wooApi again if it's null but env vars are present
      // This is a fallback, ideally it should be initialized at startup in lib/woocommerce.ts
      console.warn("[API /process-photos] wooApi was null, attempting re-initialization. This should ideally not happen.");
      try {
        const tempWooApi = (await import('@/lib/woocommerce')).wooApi;
        if (!tempWooApi) {
           console.error("[API /process-photos] CRITICAL: Re-initialization of wooApi failed. Aborting.");
           return NextResponse.json({ error: 'Server configuration error: WooCommerce API could not be initialized.' }, { status: 500 });
        }
         // If successful, the global wooApi in lib/woocommerce should now be set
         console.log("[API /process-photos] wooApi re-initialized successfully (or was already set by import).");
      } catch (initError) {
          console.error("[API /process-photos] CRITICAL: Error during dynamic import/re-initialization of wooApi:", initError);
          return NextResponse.json({ error: 'Server configuration error: WooCommerce API dynamic initialization failed.' }, { status: 500 });
      }
    } else if (!wooApi) {
        console.error("[API /process-photos] CRITICAL: WooCommerce API client (wooApi) not initialized and env vars might be missing. Aborting.");
        return NextResponse.json({ error: 'Server configuration error: WooCommerce API not available or not configured.' }, { status: 500 });
    }
    ...
    if (!photosToProcessSnapshot.empty) {
      ...
      try {
        await photoDocRef.update({ status: 'processing_image_started', progress: 5, updatedAt: admin.firestore.FieldValue.serverTimestamp(), lastMessage: 'Iniciando procesamiento de imagen.' });
        ...
        if (!photoData.originalDownloadUrl) {
            ...
            await triggerNextPhotoProcessing(batchId, originalRequestUrlString, userIdForThisPhoto, `After critical URL error for ${photoData.imageName}`);
        ...
        try {
          ...
          console.log(`[ImgProc - ${photoData.imageName}] Local file read successfully. Size: ${imageBuffer.length}`);
           ...
           await photoDocRef.update({ status: 'error_processing_image', errorMessage: `Failed to read local image file: ${readError.message.substring(0,200)}`, progress: 5, updatedAt: admin.firestore.FieldValue.serverTimestamp(), lastMessage: `Error leyendo archivo local: ${readError.message.substring(0,50)}...` });
           throw readError;
        }
        console.log(`[ImgProc - ${photoData.imageName}] Image buffer loaded. Size: ${imageBuffer.length} bytes.`);
        await photoDocRef.update({ status: 'processing_image_downloaded', progress: 10, updatedAt: admin.firestore.FieldValue.serverTimestamp(), lastMessage: 'Imagen cargada desde servidor local (backend).' });
        ...
        await photoDocRef.update({ status: 'processing_image_name_parsed', progress: 15, parsedNameData, updatedAt: admin.firestore.FieldValue.serverTimestamp(), lastMessage: 'Nombre de archivo analizado.' });
        ...
        await photoDocRef.update({ status: 'processing_image_classified', progress: 25, visualTags, updatedAt: admin.firestore.FieldValue.serverTimestamp(), lastMessage: 'Imagen clasificada (IA Stub).' });
        ...
        console.log(`[ImgProc - ${photoData.imageName}] AI: generateContentWithMiniLM START. ProductName: ${miniLMInput.productName}, Category: ${miniLMInput.category}`);
        ...
        console.log(`[ImgProc - ${photoData.imageName}] AI: generateContentWithMiniLM END. Time: ${miniLMEndTime - miniLMStartTime}ms. SEO Base='${generatedContent.seoFilenameBase}'`);
        await photoDocRef.update({ status: 'processing_image_content_generated', progress: 45, generatedContent, updatedAt: admin.firestore.FieldValue.serverTimestamp(), lastMessage: 'Contenido generado (IA Stub).' });
        ...
        if (wcMediaUploadResult && 'id' in wcMediaUploadResult && wcMediaUploadResult.id) {
          ...
          await photoDocRef.update({ status: 'error_processing_image', errorMessage: `${uploadErrorMsg.substring(0,100)} Details: ${uploadErrorDetails.substring(0,150)}`, progress: 85, updatedAt: admin.firestore.FieldValue.serverTimestamp(), lastMessage: `Error subiendo a WC: ${uploadErrorMsg.substring(0,50)}...` });
        ...
        await triggerNextPhotoProcessing(batchId, originalRequestUrlString, userIdForThisPhoto, `After processing ${photoData.imageName}`);
      }
      ...
      if (entriesReadyForWooCommerce.length > 0) {
        ...
        entriesReadyForWooCommerce.forEach(entry => {
            ...
            if (!productNameKey) { 
                console.warn(`[API /process-photos] Batch ${batchId}: Entry ${entry.id} (Image: ${entry.imageName}) missing productContext.name in product creation phase. Marking error.`);
                updateSpecificFirestoreEntries([entry.id], 'error_woocommerce_integration', { errorMessage: "Contexto de nombre de producto faltante para creación WC.", lastMessage: "Error: Falta nombre de producto para WC."});
                return;
            }
        ...
        if (numProductsToCreate > 0) {
          ...
          if(isEntireBatchTerminal) {
              ...
              await triggerNextPhotoProcessing(batchId, originalRequestUrlString, userIdForBatchOverall, `After WC product creation cycle (not terminal)`);
              const requestEndTime = Date.now();
             ...
             const allBatchEntriesFinalSnapshot = await adminDb.collection('processing_status').where('batchId', '==', batchId).get();
             const isEntireBatchTerminal = allBatchEntriesFinalSnapshot.docs.every(d => { const s = d.data().status; return s.startsWith('completed_') || s.startsWith('error_');});
             if (isEntireBatchTerminal) {
                 await createBatchCompletionNotification(batchId, userIdForBatchOverall, productCreationResults); 
             } else {
                 await triggerNextPhotoProcessing(batchId, originalRequestUrlString, userIdForBatchOverall, `No valid product groups but batch not terminal`);
             }
             const requestEndTime = Date.now();
        ...
        if (allTerminal) {
            ...
            const rawBatchDir = path.join(process.cwd(), 'public', LOCAL_UPLOAD_RAW_DIR_RELATIVE, batchId);
            const processedBatchDir = path.join(process.cwd(), 'public', LOCAL_UPLOAD_PROCESSED_DIR_RELATIVE, batchId);
            try { await fs.rm(rawBatchDir, { recursive: true, force: true }); console.log(`[Cleanup] Deleted raw batch dir: ${rawBatchDir}`); } catch (e: any) { /* ignore */ }
            try { await fs.rm(processedBatchDir, { recursive: true, force: true }); console.log(`[Cleanup] Deleted processed batch dir: ${processedBatchDir}`); } catch (e: any) { /* ignore */ }
    ...
    const originalUrlForTrigger = originalRequestUrlString; 
    if (body?.batchId && originalUrlForTrigger && originalUrlForTrigger !== "unknown_url_from_request_object") {
        console.log(`[API /process-photos] CRITICAL ERROR HANDLER: Attempting to trigger next processing for batch ${body.batchId} to avoid stall. Original URL: ${originalUrlForTrigger}`);
        await triggerNextPhotoProcessing(body.batchId, originalUrlForTrigger, body.userId, "Critical error handler self-trigger");
    } else {
        console.error(`[API /process-photos] CRITICAL ERROR HANDLER: Could not self-trigger next processing. BatchId: ${body?.batchId}, RequestURL: ${originalUrlForTrigger}`);
    }
