...
import {interpretationFlow} from '@/ai/flows/interpret-seo-analysis';
...
export async function POST(
  ...
  try {
    ...
    const interpretationResult = await runFlow(interpretationFlow, data.analysis);
    ...
    return NextResponse.json(
      {
        error: 'Fallo al obtener la interpretación del análisis',
        details: error.message,
      },
      {status: 500}
    );
